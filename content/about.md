---
title: "About"
description: ""
showToc: false
author: "Moiz Bootwala"
ShowBreadCrumbs: false
---

Hi. I'm Moiz Bootwala.

I work in offensive security as a red and purple team operator — adversary simulation, penetration testing, and detection validation across web, network, cloud, and Active Directory environments.

Outside of the day job, I like poking at security tools and tech to understand how they actually work — less about building everything from scratch and more about figuring out how to apply the right thing well. A lot of what I learn doing that ends up here.

This site is where I document the following:
- [/writeups/](/writeups/) — walkthroughs of CTF boxes I solve for fun across HackTheBox, TryHackMe, VulnHub, and more
- [/blog/](/blog/) — Research notes and technical deep-dives on whatever I've been breaking

Lately I’ve been digging into AI engineering — mostly how to use existing models and LLMs in security tooling and workflows to get more out of the work I’m already doing.

Research:
- [Exploit-DB PoC for authenticated command injection (CVE-2024-30167)](https://www.exploit-db.com/exploits/52513)

Certifications: CEH · CRTP · CRTO

Outside of security, I like gym, cars, bikes, music, and coffee.

Find me elsewhere:
- [GitHub](https://github.com/RIZZZIOM)
- [X / Twitter](https://x.com/rizzziom)
- [LinkedIn](https://www.linkedin.com/in/moiz-bootwala)
- [Email](mailto:rizzziom@gmail.com)
